...
s = s.replace("import { useEffect, useMemo, useState } from 'react';", "import { useEffect, useMemo, useState, type ComponentType, type ReactNode } from 'react';", 1)
...
s = s.replace("ReadonlyArray<readonly [string, string, React.ComponentType<{size?: number}>]>", "ReadonlyArray<readonly [string, string, ComponentType<{size?: number}>]>")
s = s.replace("icon: React.ComponentType<{size?: number}>", "icon: ComponentType<{size?: number}>")
p.write_text(s, encoding='utf-8')
print('Fixed reference dashboard TypeScript imports.')
