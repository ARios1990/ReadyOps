...
/** Simple ReadyMode generator. The logged-in ReadyMode User.Name supplies the agent automatically. */
export function ReadyModeAgentTools({ companies }: ReadyModeAgentToolsProps) {
  ...
  const output = useMemo(() => {
    if (!bookingBase || !outputType) return '';
    return outputType === 'url'
      ? buildReadyModeUrlTemplate(bookingBase)
      : buildReadyModeScript(bookingBase);
  }, [bookingBase, outputType]);
    ...
    <section className="rounded-2xl border bg-white">
      <div className="border-b p-4">
        ...
        <p className="mt-1 text-xs text-slate-500">
          Select the company. ReadyMode automatically supplies the logged-in agent from User.Name.
        </p>
      </div>
      ...
      <div className="space-y-4 p-4">
        <label className="block text-xs font-bold text-slate-500">
          ...
          <select
            value={companyId}
            onChange={event => { setCompanyId(event.target.value); setOutputType(null); }}
            className="mt-1 w-full max-w-xl rounded-lg border px-3 py-2.5 text-sm text-slate-800"
          >
        ...
        <div className="grid max-w-xl gap-2 sm:grid-cols-2">
          ...
          >
            <Code2 size={16} /> 2. Generate Script
          </button>
        </div>
