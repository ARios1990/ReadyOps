...
export function AdminPayroll() {
  ...
  async function load(preferred?: string) {
    ...
    const ps = (p.data || []) as Obj[];
    setPeriods(ps);
    setAgents((a.data || []) as Obj[]);
    setTeams((t.data || []) as Obj[]);
    const chosen = preferred || periodId || ps[0]?.id || "";
    setPeriodId(chosen);
    if (chosen) {
  ...
  const stats = useMemo(
    () => ({
      ...
      qualified: entries.reduce(
        (s, e) => s + Number(e.qualified_leads || 0),
        0,
      ),
      hours: entries.reduce((s, e) => s + Number(e.hours || 0), 0),
      commission: entries.reduce(
        (s, e) =>
          s +
          Number(e.qualified_leads || 0) * Number(e.lead_rate || 0) +
          Number(e.signed_contracts || 0) * Number(e.signed_contract_rate || 0),
        0,
      ),
      bonuses: entries.reduce((s, e) => s + Number(e.bonus || 0), 0),
      deductions: entries.reduce((s, e) => s + Number(e.deductions || 0), 0),
      total: entries.reduce((s, e) => s + Number(e.total_pay || 0), 0),
    }),
    [entries],
  );
  async function editEntry(row: Obj) {
    if (period?.status === "locked") {
      setError("This payroll period is locked.");
      return;
    }
    const hours = promptNum("Hours", row.hours);
    if (hours === null) return;
    const base = promptNum("Base Pay", row.base_pay);
    if (base === null) return;
    const lead = promptNum("Lead Rate / Commission", row.lead_rate);
    if (lead === null) return;
    const signed = promptNum("Signed Contract Rate", row.signed_contract_rate);
    if (signed === null) return;
    const bonus = promptNum("Bonus", row.bonus);
    if (bonus === null) return;
    const deductions = promptNum("Deductions", row.deductions);
    ...
    else await loadEntries(periodId);
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="readyops-ref-card overflow-hidden">
        <div className="readyops-ref-card-heading">
          <h3>Agent Payroll</h3>
          <p className="mt-1 text-xs opacity-60">
            Generate a week first. Qualified and signed-contract counts come
            from ReadyOps leads; pay rates remain configurable per agent.
          </p>
        </div>
        {loading ? (
                ...
                {entries.map((r) => {
                  ...
                  return (
                    <tr key={r.id} className="border-t">
                      ...
                      <td>{r.signed_contracts}</td>
                      <td>{money(Number(r.base_pay))}</td>
                      <td>{money(Number(r.lead_rate))}</td>
                      <td>{money(Number(r.signed_contract_rate))}</td>
                      <td>{money(Number(r.bonus))}</td>
                      <td>{money(Number(r.deductions))}</td>
                      <td className="font-black">
                        {money(Number(r.total_pay))}
                      </td>
                      <td>
                        <button
                          className="readyops-ref-secondary"
                          disabled={period?.status === "locked"}
                          onClick={() => void editEntry(r)}
                        >
                          Edit Pay
                        </button>
                      </td>
...
function promptNum(label: string, value: unknown) {
  const raw = window.prompt(label, String(value ?? 0));
