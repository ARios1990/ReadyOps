...
export function AdminReports({ initialStatusFilter = 'all' }: { initialStatusFilter?: string }) {
  ...
  async function load() {
    ...
    const firstError = c.error || a.error || t.error || l.error || ap.error || inv.error || pay.error;
  ...
  const agentRows = useMemo(() => agents.map(agent => {
    const rows = filtered.filter(r => r.lead?.agent_id === agent.id || (!r.lead?.agent_id && r.lead?.agent_name === agent.name));
    const team = teams.find(t => t.id === agent.team_id);
    const approved = rows.filter(r => r.lead?.qc_status === 'approved').length;
    return {
      name: agent.name,
  ...
  return <div className="space-y-4">
    <div className="readyops-ref-page-header">
      <div className="readyops-ref-title-row"><h2>Reports</h2><span>Operational + Financial Performance</span></div>
      <div className="readyops-ref-page-actions"><button className="readyops-ref-secondary" onClick={() => void load()}><RefreshCw size={14}/> Refresh</button><button className="readyops-ref-primary" onClick={exportCsv}><Download size={14}/> Export CSV</button></div>
    </div>

    <section className="readyops-ref-card p-4"><div className="grid gap-3 md:grid-cols-5">
      <label className="text-xs font-bold">From<input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="mt-1 w-full rounded-lg border p-2"/></label>
