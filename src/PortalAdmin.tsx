...
function initialStatusFilter(): CompanyStatusFilter {
  ...
  return ['active', 'active-package', 'pending-payment', 'all'].includes(requested || '')
...
export function PortalAdmin() {
  ...
  const [filter, setFilter] = useState<CompanyStatusFilter>(initialStatusFilter);
  ...
  const visible = useMemo(() => companies.filter(company => {
    ...
    const matchesStatus = filter === 'all'
      || (filter === 'pending-payment' && company.package?.payment_status === 'pending')
  ...
  async function openAppointmentAvailability() {
    const company = visible.find(item => item.company_id === expanded) || visible[0];
    if (!company) return;
    if (expanded !== company.company_id) await toggleCompany(company);
    window.setTimeout(() => {
      document.getElementById(`company-availability-${company.company_id}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }, 50);
  }
  ...
  const pageActions = <><button onClick={() => document.getElementById('company-signup')?.scrollIntoView({ behavior: 'smooth' })} className="readyops-ref-primary"><Plus size={14} /> Add Company</button><button disabled={!expanded && visible.length === 0} onClick={() => setManager({ companyId: expanded || visible[0]?.company_id, mode: 'locations' })} className="readyops-ref-secondary disabled:opacity-40"><Plus size={14} /> Add Location</button><button disabled={visible.length === 0} onClick={() => void openAppointmentAvailability()} className="readyops-ref-secondary disabled:opacity-40"><CalendarDays size={14} /> Availability</button><button onClick={() => document.getElementById('company-list')?.scrollIntoView({ behavior: 'smooth' })} className="readyops-ref-secondary"><PackageCheck size={14} /> Packages</button><button onClick={() => { location.href = '/qc'; }} className="readyops-ref-secondary"><ShieldCheck size={14} /> QC Queue</button></>;
