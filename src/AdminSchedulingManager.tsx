...
export function AdminSchedulingManager({
  store,
  ...
  const defaultCompanyId =
    initialCompanyId ||
    store.companies.find((company) => company.account_status === "Active")
      ?.id ||
  ...
  const [locationDraft, setLocationDraft] =
    useState<LocationDraft>(blankLocation);
  ...
  }, [companyId, company]); // eslint-disable-line react-hooks/exhaustive-deps -- store accessors are stable and adding the store object would retrigger every render
  ...
  function setLocationField<K extends keyof LocationDraft>(
    key: K,
  ...
  async function saveLocation() {
    ...
    const payload = {
      ...
      timezone:
        locationDraft.timezone.trim() ||
        inferUsTimeZone(
          locationDraft.city,
          locationDraft.state,
          locationDraft.zip_code,
        ) ||
        "America/Chicago",
      ...
      slot_interval_minutes: Math.max(
        15,
        Number(locationDraft.slot_interval_minutes) || 60,
      ),
    ...
    if (locationId) {
      const agentIdsToAdd = assignedAgentIds.filter(
        (agentId) => !savedAgentIds.includes(agentId),
      );
      const agentIdsToRemove = savedAgentIds.filter(
        (agentId) => !assignedAgentIds.includes(agentId),
      );
    ...
    setMessage(
      editingLocationId
        ? "Location updated."
        : "Location added. A separate time-slot row is now available for this area.",
    );
    setSaving(false);
  }
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="mr-auto">
              ...
              <p className="text-xs text-slate-500">
                Edit company information and create separate office/service-area
                schedules.
              </p>
            </div>
            <button
          ...
          {mode === "company" ? (
            <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <div className="mb-4">
                ...
                <p className="text-xs text-slate-500">
                  Update the information most often needed while working the
                  time-slot page.
                </p>
              </div>
              ...
              <div className="grid gap-3 md:grid-cols-2">
                ...
                <Field
                  ...
                  onChange={(value) => setCompanyField("state", value)}
                />
                <Field
                  label="Primary Contact"
                  value={companyForm.contact_name || ""}
                  onChange={(value) => setCompanyField("contact_name", value)}
                />
                <Field
                  label="Phone"
                  value={companyForm.phone || ""}
                  onChange={(value) => setCompanyField("phone", value)}
                />
                <Field
                  label="Email"
                  type="email"
                ...
                <Field
                  ...
                  onChange={(value) => setCompanyField("website", value)}
                />
                <label className="text-xs font-bold text-slate-600">
                  Status
                  <select
                    value={companyForm.account_status || "Active"}
                    onChange={(event) =>
                      setCompanyField("account_status", event.target.value)
                    }
                ...
                <TextArea
                  label="Requirements / Time-Slot Note"
                  value={companyForm.requirements_note || ""}
                  onChange={(value) =>
                    setCompanyField("requirements_note", value)
                  }
                />
                <TextArea
                  label="Internal Notes"
                  value={companyForm.notes || ""}
                  onChange={(value) => setCompanyField("notes", value)}
                />
              </div>
              ...
              <div className="mt-4 rounded-xl border border-slate-200 p-4">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {store.teams.map((team) => (
                    <label
                      key={team.id}
                      className={`cursor-pointer rounded-lg border px-3 py-2 text-xs font-bold ${companyTeamIds.includes(team.id) ? "border-blue-300 bg-blue-50 text-blue-700" : "border-slate-200 text-slate-600"}`}
                    >
                      <input
                        type="checkbox"
                        className="mr-2"
                        checked={companyTeamIds.includes(team.id)}
                        onChange={() =>
                          setCompanyTeamIds((current) =>
              ...
              <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                <div className="flex flex-wrap items-center gap-2">
                  <div className="mr-auto">
                    ...
                    <p className="text-xs text-slate-500">
                      Each location appears as its own appointment-availability
                      row and has independent weekly blocks.
                    </p>
                  </div>
                  <button
                ...
                <div className="mt-3 grid gap-2 md:grid-cols-2">
                  {companyLocations.length === 0 ? (
                    <div className="col-span-full rounded-xl border border-dashed border-slate-300 p-4 text-sm text-slate-500">
                      No locations yet. Add the first location below. Existing
                      company-wide blocked slots stay intact and apply as
                      inherited legacy blocks.
                    </div>
                  ) : (
                        ...
                        <div className="flex items-start gap-2">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="mt-1 text-[10px] font-medium text-slate-400">
                              {timeValue(location.start_time, "09:00")}–
                              {timeValue(location.end_time, "18:00")}
                            </div>
                          </div>
                        </div>
                      </button>
              ...
              <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                <div className="mb-4 flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      Use a branch, metro, territory, or office name such as
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  <Field
                    ...
                    value={locationDraft.location_label}
                    onChange={(value) =>
                      setLocationField("location_label", value)
                    }
                    placeholder="Austin / Central Texas"
                  />
                  <Field
                    label="Office Name"
                    value={locationDraft.office_name}
                    onChange={(value) => setLocationField("office_name", value)}
                  ...
                  <Field
                    ...
                    onChange={(value) => setLocationGeoField("city", value)}
                  />
                  <Field
                    label="State"
                    value={locationDraft.state}
                    onChange={(value) => setLocationGeoField("state", value)}
                    placeholder="TX"
                  />
                  <Field
                    label="ZIP"
                    value={locationDraft.zip_code}
                    onChange={(value) => setLocationGeoField("zip_code", value)}
                  />
                  <TextArea
                    label="Service Cities"
                    value={locationDraft.service_cities}
                    onChange={(value) =>
                      setLocationField("service_cities", value)
                    }
                    placeholder="Austin, Buda, Kyle"
                  />
                  <TextArea
                    label="Service ZIP Codes"
                    value={locationDraft.service_zips}
                    onChange={(value) =>
                      setLocationField("service_zips", value)
                    }
                    placeholder="78610, 78633, 78640"
                  />
                  <Field
                    label="Office Phone"
                    value={locationDraft.phone}
                    onChange={(value) => setLocationField("phone", value)}
                  />
                  <Field
                    label="Office Email"
                    type="email"
                    value={locationDraft.email}
                    onChange={(value) => setLocationField("email", value)}
                  />
                  <Field
                    label="Manager / Office Contact"
                    value={locationDraft.manager_name}
                    onChange={(value) =>
                      setLocationField("manager_name", value)
                    }
                  />
                  <label className="text-xs font-bold text-slate-600">
                    Time Zone{" "}
                    ...
                    >
                      {!locationDraft.timezone && (
                        <option value="">
                          Enter City / State / ZIP to auto-detect
                        </option>
                      )}
                      {READYOPS_TIME_ZONES.map((zone) => (
                    ...
                    <span className="mt-1 block text-[10px] font-normal text-slate-400">
                      ReadyOps fills this from the location. You can change it
                      anytime if the service area crosses a time-zone boundary.
                    </span>
                  </label>
                </div>
                ...
                <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {DAYS.map((day) => (
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={locationDraft.available_days.includes(day)}
                  ...
                  <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
                    <Field
                      label="Start Time"
                      type="time"
                      value={locationDraft.start_time}
                      onChange={(value) =>
                        setLocationField("start_time", value)
                      }
                    />
                    <Field
                      label="End Time"
                      type="time"
                      value={locationDraft.end_time}
                      onChange={(value) => setLocationField("end_time", value)}
                    />
                    <Field
                      label="Slot Minutes"
                      type="number"
                      value={locationDraft.slot_interval_minutes}
                      onChange={(value) =>
                        setLocationField("slot_interval_minutes", value)
                      }
                    />
                    <Field
                      label="Max / Hour"
                      type="number"
                      value={locationDraft.max_per_hour}
                      onChange={(value) =>
                        setLocationField("max_per_hour", value)
                      }
                    />
                    <Field
                      label="Max / Day"
                      type="number"
                      value={locationDraft.max_per_day}
                      onChange={(value) =>
                        setLocationField("max_per_day", value)
                      }
                    />
                  </div>
                </div>
                ...
                <div className="mt-4">
                  <TextArea
                    ...
                    onChange={(value) => setLocationField("notes", value)}
                    placeholder="Office-specific scheduling or service-area notes..."
                  />
                </div>
