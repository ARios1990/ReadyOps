...
export function AdminInvoices() {
  ...
  const [form, setForm] = useState<Obj>({
    company_id: "",
    period_start: isoDate(first),
    period_end: isoDate(now),
    billing_type: "per_lead",
    rate: "",
    additional: "0",
    discount: "0",
    due_date: isoDate(new Date(Date.now() + 7 * 86400000)),
  });
  async function load() {
    setLoading(true);
    setError("");
    ...
    if (i.error || c.error)
      setError((i.error || c.error)?.message || "Unable to load invoices");
    setRows((i.data || []) as Obj[]);
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      paid: rows
        .filter((r) => String(r.updated_at || "").startsWith(month))
        .reduce((s, r) => s + Number(r.amount_paid || 0), 0),
      overdue: rows.filter((r) => r.status === "overdue").length,
      draft: rows.filter((r) => r.status === "draft").length,
      month: rows.filter((r) => String(r.created_at || "").startsWith(month))
        .length,
    };
  }, [rows]);
  async function createInvoice() {
    ...
    setError("");
    const { error: e } = await supabase.rpc("create_readyops_invoice", {
      p_company_id: form.company_id,
      p_period_start: form.period_start,
      p_period_end: form.period_end,
      p_billing_type: form.billing_type,
      p_rate: Number(form.rate || 0),
      p_additional_charges: Number(form.additional || 0),
  ...
  async function updateStatus(id: string, status: string) {
    ...
    if (status === "paid") {
      ...
      patch.amount_paid = Number(row?.total || 0);
    }
  ...
  async function addPayment(row: Obj) {
    const raw = window.prompt(
      `Payment amount for ${row.invoice_number}`,
    ...
    const { error: e1 } = await supabase
      ...
      .insert({ invoice_id: row.id, amount });
    if (e1) {
      setError(e1.message);
      return;
    }
    const paid = Number(row.amount_paid || 0) + amount;
    const status = paid >= Number(row.total || 0) ? "paid" : "partial";
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        <Kpi label="Outstanding Balance" value={money(stats.outstanding)} />
        <Kpi label="Paid This Month" value={money(stats.paid)} />
        <Kpi label="Overdue" value={stats.overdue} />
        <Kpi label="Draft Invoices" value={stats.draft} />
        <Kpi label="Invoices This Month" value={stats.month} />
      </section>
      <section className="readyops-ref-card overflow-hidden">
        <div className="readyops-ref-card-heading flex items-center gap-2">
          <WalletCards size={18} />
          <h3>Invoices</h3>
        </div>
        {loading ? (
                ...
                {rows.length ? (
                  rows.map((r) => (
                    <tr key={r.id} className="border-t">
                      ...
                      <td>{r.billable_leads}</td>
                      <td>{money(Number(r.total))}</td>
                      <td>{money(Number(r.amount_paid))}</td>
                      <td className="font-bold">{money(Number(r.balance))}</td>
                      <td>{r.due_date || "—"}</td>
                      <td>
                        <select
                          value={r.status}
                          onChange={(e) =>
                            void updateStatus(r.id, e.target.value)
                          }
                  ...
                  <tr>
                    <td colSpan={10} className="p-10 text-center opacity-50">
                      No invoices yet. Click Create Invoice to generate the
                      first one from ReadyOps lead data.
                    </td>
                  </tr>
                )}
              </tbody>
      ...
      {showCreate && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4">
          <div className="readyops-ref-card w-full max-w-2xl p-5">
            <div className="mb-4 flex items-center justify-between">
              <div>
                <h3 className="text-lg font-bold">Create Invoice</h3>
                <p className="text-xs opacity-60">
                  Billable lead statuses default to Good and Signed Contract.
                  Already billed Lead IDs are excluded.
                </p>
              </div>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <Field label="Billing Type">
                ...
                >
                  ...
                  <option value="package_upfront">Package Upfront</option>
