...
interface Appointment { id: string; appointment_date: string; start_time: string; end_time: string; status: string; rep_status: string; attendance_status: string; inspection_status: string; sales_outcome: string; client_status: string; inspector_notes: string | null; representative_id: string | null; representative_name: string | null; location_label: string | null; lead: LeadRecord; latest_checkin: { verified?: boolean; distance_m?: number; checked_in_at?: string } | null; }
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <main className="mx-auto max-w-7xl px-4 py-5 sm:px-6">
        ...
        {tab === 'appointments' && <section className="space-y-3"><div><h2 className="text-lg font-bold">Appointments</h2><p className="text-sm text-slate-500">Homeowner preview, rep assignment, attendance and appointment outcome.</p></div>{data.appointments.length === 0 ? <Empty text="No appointments in this date window." /> : data.appointments.map(appt => <article key={appt.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><button onClick={() => setSelectedLead(appt)} className="text-left"><p className="text-xs font-bold text-blue-600">{formatDateLong(appt.appointment_date)} • {formatTime(appt.start_time)}</p><h3 className="mt-1 font-bold">{appt.lead.full_name}</h3><p className="text-sm text-slate-600">{appt.lead.phone_number} • {appt.lead.address}</p><p className="mt-1 text-xs text-slate-400">{appt.location_label || 'Company-wide'} • {appt.attendance_status.replace(/_/g, ' ')}</p></button><div className="flex flex-col gap-2 lg:items-end"><AppointmentWeatherBadge date={appt.appointment_date} city={appt.lead.city} state={appt.lead.state} zip={appt.lead.zip_code} /><div className="grid gap-2 sm:grid-cols-2 lg:min-w-[420px]"><select value={appt.representative_id || ''} onChange={e => void assignRep(appt.id, e.target.value)} disabled={busy} className="rounded-xl border border-slate-200 px-3 py-2 text-sm"><option value="">Unassigned</option>{data.representatives.filter(rep => rep.active).map(rep => <option key={rep.id} value={rep.id}>{rep.name}</option>)}</select><select value={appt.status} onChange={e => void updateAppointmentStatus(appt.id, e.target.value)} disabled={busy} className="rounded-xl border border-slate-200 px-3 py-2 text-sm"><option value="confirmed">Confirmed</option><option value="assigned">Assigned</option><option value="completed">Completed</option><option value="cancelled">Cancelled</option></select><select value={appt.client_status || 'pending'} onChange={e => void updateLeadOutcome(appt, e.target.value)} disabled={busy} className="rounded-xl border border-slate-200 px-3 py-2 text-sm"><option value="pending">Pending</option><option value="good">Good</option><option value="bad">Bad</option><option value="no_show">No Show</option><option value="reschedule">Reschedule</option><option value="follow_up">Follow Up</option><option value="signed_contract">Signed Contract</option></select></div></div></div></article>)}</section>}
