...
import { AlertTriangle, BarChart3, CalendarDays, CheckCircle2, Clipboard, Download, ExternalLink, FileSpreadsheet, Loader2, Plus, RefreshCw, Save, ShieldCheck, ThumbsDown, Trash2, UserRoundCheck, Users, UserX } from 'lucide-react';
import { supabase } from './supabase';
import { PortalFormField, PortalFormSection } from './DynamicLeadForm';
import { addDays, copyText, dateInTimeZone, formatDateLong, formatTime, localDate, rpcError, startOfWeek } from './portalUtils';
...
interface Appointment { id: string; appointment_date: string; start_time: string; end_time: string; status: string; canonical_status?: string; rep_status: string; attendance_status: string; inspection_status: string; sales_outcome: string; client_status: string; inspector_notes: string | null; representative_id: string | null; representative_name: string | null; location_label: string | null; lead: LeadRecord; latest_checkin: { verified?: boolean; distance_m?: number; checked_in_at?: string } | null; }
...
interface CompanyPortalData {
  company: { id: string; name: string; state: string | null; email: string | null; phone: string | null; logo_path?: string | null };
...
interface CompanyDashboardSummary {
  company: { id: string; name: string; state: string | null; logo_path: string | null };
  performance: { total_leads: number; good_inspected: number; signed_contracts: number; no_shows: number; bad_leads: number; inspection_rate: number; close_rate: number };
  active_package: null | { id: string; package_number: number | null; package_name: string | null; lead_target: number; amount_per_lead: number | null; payment_status: string; start_date: string | null; delivered_leads: number; remaining_leads: number; completion_percentage: number; agreement_type?: string | null };
  package_history: Array<Record<string, unknown>>;
  last_updated_at: string | null;
}

type Tab = 'appointments' | 'schedule' | 'requirements' | 'forms' | 'reps' | 'audit' | 'reports';
...
export function CompanyPortal({ companyId, token }: { companyId: string; token: string }) {
  ...
  const load = useCallback(async () => {
    ...
    const [portalResult, dashboardResult] = await Promise.all([
      supabase.rpc('get_company_management_portal', { p_company_id: companyId, p_access_token: token, p_start_date: windowStart, p_end_date: windowEnd }),
      supabase.rpc('get_company_management_dashboard_summary', { p_company_id: companyId, p_access_token: token }),
    ]);
    ...
    if (rpcErr) {
      ...
      setData(portal);
      setSelectedDay(current => current === localDate(new Date())
        ? dateInTimeZone(new Date(), portal.settings.timezone || 'America/Chicago')
        : current);
      ...
      setDashboard(dashboardResult.error ? null : dashboardResult.data as CompanyDashboardSummary);
    }
    setLoading(false);
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <header className="sticky top-0 z-30 border-b bg-white/95 backdrop-blur"><div className="mx-auto flex max-w-[1500px] items-center justify-between gap-4 px-4 py-4 sm:px-6"><div className="flex min-w-0 items-center gap-5"><img src={READYOPS_LOGO_DATA_URI} alt="ReadyOps" className="h-10 w-auto"/><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-slate-500">Company Management</p><h1 className="truncate text-lg font-black text-slate-950">{data.company.name}</h1></div><CompanyLogo company={dashboard?.company || data.company} /></div><div className="flex items-center gap-2"><span className="hidden rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-700 sm:inline">Secure Link</span><button onClick={() => void load()} className="rounded-lg border bg-white p-2 text-slate-700"><RefreshCw size={16} /></button></div></div></header>
      ...
      <main className="mx-auto max-w-[1500px] px-4 py-5 sm:px-6">
        ...
        <section className="mb-5 grid gap-3 md:grid-cols-2">
          ...
          <LinkCard title="Company Admin Link" value={companyLink} onCopy={() => void copyText(companyLink)} onOpen={() => window.open(companyLink, '_blank', 'noopener,noreferrer')} privateLink />
        </section>

        <nav className="mb-5 flex gap-1 overflow-x-auto rounded-xl border border-slate-200 bg-white p-1">
          {([['appointments','Appointments',CalendarDays],['schedule','Schedule',CalendarDays],['requirements','Requirements',ShieldCheck],['forms','Forms',Clipboard],['reps','Reps',Users],['audit','Audit',ShieldCheck],['reports','Reports',BarChart3]] as [Tab,string,typeof CalendarDays][]).map(([key,label,Icon]) => <button key={key} onClick={() => setTab(key)} className={`inline-flex shrink-0 items-center gap-1.5 rounded-lg px-3 py-2 text-xs font-bold ${tab === key ? 'bg-blue-600 text-white' : 'text-slate-600 hover:bg-slate-50'}`}><Icon size={14} />{label}</button>)}
        </nav>

        {tab === 'appointments' && <CompanyAppointmentsDashboard data={data} dashboard={dashboard} selectedDay={selectedDay} setSelectedDay={setSelectedDay} busy={busy} openLead={setSelectedLead} assignRep={assignRep} updateAppointmentStatus={updateAppointmentStatus} updateLeadOutcome={updateLeadOutcome} />}
        ...
        {tab === 'schedule' && <section className="space-y-5"><div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="font-bold">Weekly Schedule</h2><p className="text-sm text-slate-500">Edit availability by company or service area.</p></div><div><label className="mb-1 block text-xs font-semibold text-slate-500">Schedule Scope</label><select value={scheduleLocation} onChange={e => setScheduleLocation(e.target.value)} className="rounded-xl border border-slate-200 px-3 py-2 text-sm"><option value="">Company-wide</option>{data.locations.map(loc => <option key={loc.id} value={loc.id}>{loc.location_label}{loc.state ? `, ${loc.state}` : ''}</option>)}</select></div></div><div className="mb-2 hidden rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-[11px] font-black uppercase tracking-wide text-slate-500 md:grid md:grid-cols-[110px_90px_repeat(5,1fr)_70px] md:items-center md:gap-2">
  <span>Day</span>
  <span>Open</span>
  ...
  <span className="text-center">Save</span>
        ...
        {tab === 'reports' && <CompanyReports data={data} dashboard={dashboard} />}
...
function CompanyAppointmentsDashboard({ data, dashboard, selectedDay, setSelectedDay, busy, openLead, assignRep, updateAppointmentStatus, updateLeadOutcome }: { data: CompanyPortalData; dashboard: CompanyDashboardSummary | null; selectedDay: string; setSelectedDay: (value: string) => void; busy: boolean; openLead: (appointment: Appointment) => void; assignRep: (appointmentId: string, repId: string) => Promise<void>; updateAppointmentStatus: (appointmentId: string, status: string) => Promise<void>; updateLeadOutcome: (appointment: Appointment, clientStatus: string) => Promise<void> }) {
  ...
  const days = Array.from({ length: 7 }, (_, index) => localDate(addDays(startOfWeek(new Date(`${selectedDay}T12:00:00`)), index)));
  const selectedAppointments = delivered.filter(appointment => appointment.appointment_date === selectedDay).sort((a, b) => a.start_time.localeCompare(b.start_time));
  return <section className="space-y-4">
    <div className="grid gap-3 xl:grid-cols-[1.65fr_1.1fr_250px]"><section className="rounded-2xl border bg-white p-3 shadow-sm"><h2 className="mb-3 font-black">Company Performance</h2><div className="grid grid-cols-2 gap-2 lg:grid-cols-6"><PerformanceCard tone="blue" label="Leads This Day" value={selectedAppointments.length} suffix={formatDateLong(selectedDay)} icon={<CalendarDays size={18} />} /><PerformanceCard tone="blue" label="Total Leads" value={performance.total_leads} icon={<Users size={18} />} /><PerformanceCard tone="green" label="Good / Inspected" value={performance.good_inspected} suffix={percent(performance.good_inspected, performance.total_leads)} icon={<UserRoundCheck size={18} />} /><PerformanceCard tone="purple" label="Signed Contracts" value={performance.signed_contracts} suffix={percent(performance.signed_contracts, performance.total_leads)} icon={<Clipboard size={18} />} /><PerformanceCard tone="orange" label="No Shows" value={performance.no_shows} suffix={percent(performance.no_shows, performance.total_leads)} icon={<UserX size={18} />} /><PerformanceCard tone="red" label="Bad Leads" value={performance.bad_leads} suffix={percent(performance.bad_leads, performance.total_leads)} icon={<ThumbsDown size={18} />} /></div><div className="mt-3 grid grid-cols-2 gap-2"><div className="rounded-xl border bg-slate-50 p-3"><p className="text-xs font-bold text-slate-500">Inspection Rate</p><strong className="mt-1 block text-xl text-blue-700">{Number(performance.inspection_rate || 0).toFixed(1)}%</strong></div><div className="rounded-xl border bg-slate-50 p-3"><p className="text-xs font-bold text-slate-500">Close Rate</p><strong className="mt-1 block text-xl text-blue-700">{Number(performance.close_rate || 0).toFixed(1)}%</strong></div></div></section>
      <section className="rounded-2xl border bg-white p-4 shadow-sm"><h2 className="font-black">Lead Packages</h2>{pkg ? <><div className="mt-3 flex items-start justify-between gap-3"><div><p className="font-bold">{pkg.package_name || `Package #${pkg.package_number || ''}`}</p><span className="mt-1 inline-flex rounded-full bg-emerald-50 px-2 py-1 text-[10px] font-bold text-emerald-700">Active</span><p className="mt-2 text-[10px] text-slate-500">Started {pkg.start_date ? formatDateLong(pkg.start_date) : '—'}</p></div><div className="grid grid-cols-3 gap-4 text-center"><PackageNumber value={pkg.lead_target} label="Purchased" /><PackageNumber value={pkg.delivered_leads} label="Delivered" /><PackageNumber value={pkg.remaining_leads} label="Remaining" /></div></div><div className="mt-4 h-2 overflow-hidden rounded-full bg-slate-200"><span className="block h-full rounded-full bg-emerald-500" style={{ width: `${Math.min(100, Number(pkg.completion_percentage || 0))}%` }} /></div><p className="mt-1 text-right text-[10px] font-bold text-slate-500">{Number(pkg.completion_percentage || 0).toFixed(0)}% Complete</p><div className="mt-4 flex gap-2"><button disabled className="rounded-lg bg-blue-600 px-3 py-2 text-xs font-bold text-white opacity-60"><Plus size={13} className="mr-1 inline" /> Start New Package</button><span className="rounded-lg border px-3 py-2 text-xs font-bold">{dashboard?.package_history.length || 0} in History</span></div></> : <div className="mt-4 rounded-xl border border-dashed p-6 text-center text-sm text-slate-400">No active package.</div>}</section>
      <section className="rounded-2xl border bg-white p-4 shadow-sm"><h2 className="font-black">Client Agreement</h2><p className="mt-8 text-xs font-bold text-slate-600">{pkg?.agreement_type?.replace(/_/g, ' ') || 'Paid Per Lead'}</p><strong className="mt-2 block text-lg">{pkg?.amount_per_lead != null ? `$${Number(pkg.amount_per_lead).toLocaleString()} / lead` : 'Not configured'}</strong><button disabled className="mt-8 w-full rounded-lg border px-3 py-2 text-xs font-bold opacity-60">Agreement managed by ReadyOps</button></section></div>

    <section className="rounded-2xl border bg-white p-3 shadow-sm"><div className="flex flex-wrap items-center gap-2"><div className="mr-auto"><h2 className="font-black">Lead Data Tools</h2><p className="text-[10px] text-slate-500">Company-approved and delivered appointments only</p></div><button disabled title="Configure a Google Sheets connection in ReadyOps Admin" className="rounded-lg border px-3 py-2 text-xs font-bold opacity-60"><FileSpreadsheet size={14} className="mr-1 inline text-emerald-600" /> Sync Google Sheets</button><button disabled title="Configure an Excel connection in ReadyOps Admin" className="rounded-lg border px-3 py-2 text-xs font-bold opacity-60"><FileSpreadsheet size={14} className="mr-1 inline text-emerald-600" /> Sync Excel</button><button onClick={() => downloadAppointments(delivered, 'csv')} className="rounded-lg border px-3 py-2 text-xs font-bold"><Download size={14} className="mr-1 inline" /> Download CSV</button><button onClick={() => downloadAppointments(delivered, 'excel')} className="rounded-lg border px-3 py-2 text-xs font-bold"><FileSpreadsheet size={14} className="mr-1 inline text-emerald-600" /> Download Excel</button><span className="ml-2 text-[10px] font-semibold text-slate-500">Last updated: {dashboard?.last_updated_at ? new Date(dashboard.last_updated_at).toLocaleString() : 'Live'}</span></div></section>

    <section className="rounded-2xl border bg-white p-3 shadow-sm"><p className="mb-2 text-xs font-bold text-slate-500">Select a day to view its leads</p><div className="grid grid-cols-2 gap-2 md:grid-cols-4 xl:grid-cols-7">{days.map(day => { const date = new Date(`${day}T12:00:00`); const count = delivered.filter(appointment => appointment.appointment_date === day).length; const active = day === selectedDay; return <button key={day} onClick={() => setSelectedDay(day)} className={`rounded-xl border p-3 text-left transition ${active ? 'border-blue-600 bg-blue-600 text-white shadow-md' : 'bg-white hover:border-blue-300'}`}><span className="text-xs font-bold">{date.toLocaleDateString(undefined, { weekday: 'long' })}</span><span className="float-right text-[10px]">{date.toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}</span><div className="mt-3 flex items-center justify-between text-xs"><span>{count} Lead{count === 1 ? '' : 's'}</span><CalendarDays size={14} /></div></button>; })}</div></section>

    <div className="flex items-center justify-between"><h2 className="text-lg font-black">{new Date(`${selectedDay}T12:00:00`).toLocaleDateString(undefined, { weekday: 'long' })} Leads</h2><span className="text-xs font-bold">{selectedAppointments.length} lead{selectedAppointments.length === 1 ? '' : 's'} for this day</span></div>
    {selectedAppointments.length === 0 ? <Empty text="No approved appointments were sent for this date." /> : selectedAppointments.map(appointment => <CompanyAppointmentRow key={appointment.id} appointment={appointment} representatives={data.representatives} busy={busy} openLead={openLead} assignRep={assignRep} updateAppointmentStatus={updateAppointmentStatus} updateLeadOutcome={updateLeadOutcome} />)}
  </section>;
}

function CompanyAppointmentRow({ appointment, representatives, busy, openLead, assignRep, updateAppointmentStatus, updateLeadOutcome }: { appointment: Appointment; representatives: Representative[]; busy: boolean; openLead: (appointment: Appointment) => void; assignRep: (appointmentId: string, repId: string) => Promise<void>; updateAppointmentStatus: (appointmentId: string, status: string) => Promise<void>; updateLeadOutcome: (appointment: Appointment, clientStatus: string) => Promise<void> }) {
  const canonical = appointment.canonical_status || appointment.client_status || appointment.status;
  return <article className="rounded-2xl border bg-white p-4 shadow-sm"><div className="grid gap-4 xl:grid-cols-[1.25fr_1fr_1fr_1fr]"><button onClick={() => openLead(appointment)} className="text-left"><p className="text-xs font-bold text-blue-600">{formatDateLong(appointment.appointment_date)} • {formatTime(appointment.start_time)}</p><h3 className="mt-1 font-black">{appointment.lead.full_name}</h3><p className="text-xs text-slate-600">{appointment.lead.phone_number} • {appointment.lead.address}</p><p className="mt-1 text-[10px] text-slate-400">{appointment.location_label || 'Company-wide'} • {appointment.lead.qualification_status.replace(/_/g, ' ')}</p></button><div className="border-l pl-4"><p className="text-[10px] font-bold text-slate-500">Inspector / Lead Status</p><button onClick={() => openLead(appointment)} className="mt-2"><StatusChip status={canonical} /></button><p className="mt-2 text-[10px] text-slate-400">Lead status mirrors inspector status</p></div><div className="space-y-2 border-l pl-4"><p className="text-[10px] font-bold text-slate-500">Rep Assignment</p><select value={appointment.representative_id || ''} onChange={event => void assignRep(appointment.id, event.target.value)} disabled={busy} className="w-full rounded-lg border px-3 py-2 text-xs"><option value="">Unassigned</option>{representatives.filter(rep => rep.active).map(rep => <option key={rep.id} value={rep.id}>{rep.name}</option>)}</select><select value={appointment.client_status || 'pending'} onChange={event => void updateLeadOutcome(appointment, event.target.value)} disabled={busy} className="w-full rounded-lg border px-3 py-2 text-xs"><option value="pending">Pending</option><option value="good">Good</option><option value="bad">Bad</option><option value="no_show">No Show</option><option value="reschedule">Reschedule</option><option value="follow_up">Follow Up</option><option value="signed_contract">Signed Contract</option></select></div><div className="border-l pl-4"><div className="flex items-center justify-between"><p className="text-[10px] font-bold text-slate-500">Appointment</p><AppointmentWeatherBadge date={appointment.appointment_date} city={appointment.lead.city} state={appointment.lead.state} zip={appointment.lead.zip_code} /></div><select value={appointment.status} onChange={event => void updateAppointmentStatus(appointment.id, event.target.value)} disabled={busy} className="mt-2 w-full rounded-lg border px-3 py-2 text-xs"><option value="confirmed">Confirmed</option><option value="assigned">Assigned</option><option value="completed">Completed</option><option value="cancelled">Cancelled</option></select></div></div></article>;
}

function CompanyReports({ data, dashboard }: { data: CompanyPortalData; dashboard: CompanyDashboardSummary | null }) {
  const performance = dashboard?.performance || performanceFromAppointments(data.appointments);
  const rows = [['Good / Inspected', performance.good_inspected, 'bg-emerald-500'], ['Signed Contracts', performance.signed_contracts, 'bg-violet-500'], ['No Shows', performance.no_shows, 'bg-orange-500'], ['Bad Leads', performance.bad_leads, 'bg-red-500']] as const;
  return <section className="grid gap-4 lg:grid-cols-2"><div className="rounded-2xl border bg-white p-5 shadow-sm"><h2 className="font-black">Lead Outcomes</h2><p className="text-xs text-slate-500">Based only on approved appointments delivered to this company.</p><div className="mt-5 space-y-4">{rows.map(([label, value, color]) => <div key={label}><div className="mb-1 flex justify-between text-xs font-bold"><span>{label}</span><span>{value}</span></div><div className="h-2 overflow-hidden rounded-full bg-slate-100"><span className={`block h-full ${color}`} style={{ width: `${Math.min(100, (Number(value) / Math.max(1, performance.total_leads)) * 100)}%` }} /></div></div>)}</div></div><div className="rounded-2xl border bg-white p-5 shadow-sm"><h2 className="font-black">Performance Summary</h2><div className="mt-5 grid grid-cols-2 gap-3"><ReportValue label="Delivered Leads" value={performance.total_leads} /><ReportValue label="Inspection Rate" value={`${Number(performance.inspection_rate).toFixed(1)}%`} /><ReportValue label="Close Rate" value={`${Number(performance.close_rate).toFixed(1)}%`} /><ReportValue label="Package Remaining" value={dashboard?.active_package?.remaining_leads ?? '—'} /></div></div></section>;
}

function CompanyLogo({ company }: { company: { name: string; logo_path?: string | null } }) { const directLogo = company.logo_path && /^(https?:|data:)/.test(company.logo_path) ? company.logo_path : null; return <div className="hidden h-16 w-20 shrink-0 items-center justify-center overflow-hidden rounded-xl border bg-white shadow-sm md:flex">{directLogo ? <img src={directLogo} alt={`${company.name} logo`} className="h-full w-full object-contain p-1" /> : <span className="text-center text-xs font-black text-slate-700">{company.name.split(/\s+/).slice(0, 2).map(word => word[0]).join('').toUpperCase()}</span>}</div>; }
function PerformanceCard({ tone, label, value, suffix, icon }: { tone: 'blue' | 'green' | 'purple' | 'orange' | 'red'; label: string; value: number; suffix?: string; icon: React.ReactNode }) { const colors = { blue: 'border-blue-100 bg-blue-50 text-blue-700', green: 'border-emerald-100 bg-emerald-50 text-emerald-700', purple: 'border-violet-100 bg-violet-50 text-violet-700', orange: 'border-orange-100 bg-orange-50 text-orange-700', red: 'border-red-100 bg-red-50 text-red-700' }; return <div className={`rounded-xl border p-3 ${colors[tone]}`}><div className="flex items-center justify-between"><span className="text-[10px] font-bold text-slate-600">{label}</span>{icon}</div><strong className="mt-2 block text-2xl text-slate-950">{value}</strong>{suffix && <span className="text-[10px] font-bold">{suffix}</span>}</div>; }
function PackageNumber({ value, label }: { value: number; label: string }) { return <div><strong className="block text-lg">{value}</strong><span className="text-[9px] text-slate-500">{label}</span></div>; }
function ReportValue({ label, value }: { label: string; value: React.ReactNode }) { return <div className="rounded-xl bg-slate-50 p-4"><p className="text-xs font-bold text-slate-500">{label}</p><strong className="mt-1 block text-2xl text-blue-700">{value}</strong></div>; }
function StatusChip({ status }: { status: string }) { const normalized = status.replace(/_/g, ' '); const style = status === 'good_inspected' || status === 'signed_contract' || status === 'good' ? 'bg-emerald-50 text-emerald-700' : status === 'bad' ? 'bg-red-50 text-red-700' : status === 'no_show' || status === 'rescheduled' || status === 'reschedule' ? 'bg-orange-50 text-orange-700' : 'bg-blue-50 text-blue-700'; return <span className={`inline-flex rounded-lg px-3 py-2 text-xs font-bold capitalize ${style}`}>{normalized}</span>; }
function performanceFromAppointments(appointments: Appointment[]): CompanyDashboardSummary['performance'] { const total = appointments.length; const good = appointments.filter(item => ['good_inspected', 'good'].includes(item.canonical_status || item.client_status)).length; const signed = appointments.filter(item => ['signed_contract'].includes(item.canonical_status || item.client_status)).length; const noShows = appointments.filter(item => ['no_show'].includes(item.canonical_status || item.client_status)).length; const bad = appointments.filter(item => ['bad'].includes(item.canonical_status || item.client_status)).length; return { total_leads: total, good_inspected: good, signed_contracts: signed, no_shows: noShows, bad_leads: bad, inspection_rate: total ? ((good + signed) / total) * 100 : 0, close_rate: good + signed ? (signed / (good + signed)) * 100 : 0 }; }
function percent(value: number, total: number): string { return `${total ? ((value / total) * 100).toFixed(1) : '0.0'}% of total`; }
function downloadAppointments(appointments: Appointment[], format: 'csv' | 'excel') { const rows = [['Date', 'Time', 'Homeowner', 'Phone', 'Address', 'Location', 'Status', 'Representative'], ...appointments.map(item => [item.appointment_date, item.start_time, item.lead.full_name, item.lead.phone_number, item.lead.address, item.location_label || 'Company-wide', item.canonical_status || item.client_status || item.status, item.representative_name || 'Unassigned'])]; const separator = format === 'excel' ? '\t' : ','; const quote = (value: string) => format === 'excel' ? value.replace(/\t|\r?\n/g, ' ') : `"${value.replace(/"/g, '""')}"`; const blob = new Blob([rows.map(row => row.map(value => quote(String(value))).join(separator)).join('\n')], { type: format === 'excel' ? 'application/vnd.ms-excel;charset=utf-8' : 'text/csv;charset=utf-8' }); const url = URL.createObjectURL(blob); const anchor = document.createElement('a'); anchor.href = url; anchor.download = `readyops-company-leads.${format === 'excel' ? 'xls' : 'csv'}`; anchor.click(); URL.revokeObjectURL(url); }
...
function LeadModal({ appointment, companyId, token, onClose }: { appointment: Appointment; companyId: string; token: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/40 p-4" onClick={onClose}>
      <div className="mx-auto my-8 max-w-3xl rounded-2xl bg-white p-5 shadow-2xl" onClick={event => event.stopPropagation()}>
        ...
        <div className="mt-3">
          <AppointmentWeatherBadge date={appointment.appointment_date} city={lead.city} state={lead.state} zip={lead.zip_code} size="lg" />
        </div>
        <div className="mt-4">
