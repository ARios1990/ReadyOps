...
type AdminSection = 'overview' | 'qc' | 'companies' | 'leads' | 'appointments' | 'staff' | 'teams' | 'active-users' | 'reports' | 'invoices' | 'payroll' | 'settings';
...
export function AdminWorkspaceShell({ active, title, subtitle, actions, children }: Props) {
  ...
  return <div className={shellClass}>
    <button type="button" className="readyops-ref-sidebar-backdrop" aria-label="Close navigation" onClick={() => setMobileOpen(false)} />
    <aside className="readyops-ref-sidebar">
      <button className="readyops-ref-wordmark" onClick={() => navigate('/')} aria-label="Ready Ops home">
    ...
    <div className="readyops-ref-workspace">
      <div className="readyops-ref-scene" aria-hidden="true"><AdminSkyline /><div className="readyops-ref-watermark">R</div></div>
        ...
        <div className="readyops-ref-top-actions">
          ...
          <div className="relative">
            <button className="readyops-ref-user" onClick={() => setUserMenu(value => !value)}>
              ...
              <span className="hidden sm:inline">{profile?.display_name || 'Admin'}</span><ChevronDown size={13} />
            </button>
      ...
      <main className="readyops-ref-main">
        <div className="readyops-ref-page-header">
          <div className="readyops-ref-title-row"><div><h2>{title}</h2><p className="mt-1 text-xs text-slate-500">{subtitle}</p></div></div>
...
function AdminNavGroup({ title, items, active, collapsed, onSelect }: { title: string; items: readonly SidebarItem[]; active: AdminSection; collapsed: boolean; onSelect: (path: string) => void }) {
  return <div className="readyops-ref-nav-group">
    <p>{collapsed ? '•' : title}</p>
    {items.map(([key, label, Icon, path]) => <button
      key={key}
...
function AdminSkyline() {
  return <svg viewBox="0 0 1400 260" preserveAspectRatio="none" aria-hidden="true"><g fill="currentColor" opacity=".35"><path d="M0 230h40v-52h18v52h33v-88h28v88h18v-36h28v36h44v-112h31v112h20v-70h32v70h30v-150h15v-38h14v38h18v150h28v-94h27v94h42v-124h36v124h18v-58h30v58h54v-175h17v-34h12v34h16v175h30v-92h36v92h29v-55h31v55h47v-148h12v-29h10v29h13v148h23v-84h33v84h43v-116h28v116h26v-48h30v48h42v-133h31v133h39v-72h25v72h56v-102h30v102h22v-60h31v60h40v30H0z" /></g></svg>;
}
