...
/* Remove duplicate and over-broad policies created during earlier portal iterations. */
...
/* Masters Ready administrators may directly manage operational tables. */
CREATE POLICY company_portal_settings_admin_select
ON public.company_portal_settings FOR SELECT TO authenticated
USING (public.portal_is_admin());
CREATE POLICY company_portal_settings_admin_insert
ON public.company_portal_settings FOR INSERT TO authenticated
WITH CHECK (public.portal_is_admin());
CREATE POLICY company_portal_settings_admin_update
ON public.company_portal_settings FOR UPDATE TO authenticated
USING (public.portal_is_admin()) WITH CHECK (public.portal_is_admin());
CREATE POLICY company_portal_settings_admin_delete
ON public.company_portal_settings FOR DELETE TO authenticated
USING (public.portal_is_admin());

CREATE POLICY company_schedule_rules_admin_select
ON public.company_schedule_rules FOR SELECT TO authenticated
USING (public.portal_is_admin());
CREATE POLICY company_schedule_rules_admin_insert
ON public.company_schedule_rules FOR INSERT TO authenticated
WITH CHECK (public.portal_is_admin());
CREATE POLICY company_schedule_rules_admin_update
ON public.company_schedule_rules FOR UPDATE TO authenticated
USING (public.portal_is_admin()) WITH CHECK (public.portal_is_admin());
CREATE POLICY company_schedule_rules_admin_delete
ON public.company_schedule_rules FOR DELETE TO authenticated
USING (public.portal_is_admin());

CREATE POLICY company_schedule_exceptions_admin_select
ON public.company_schedule_exceptions FOR SELECT TO authenticated
USING (public.portal_is_admin());
CREATE POLICY company_schedule_exceptions_admin_insert
ON public.company_schedule_exceptions FOR INSERT TO authenticated
WITH CHECK (public.portal_is_admin());
CREATE POLICY company_schedule_exceptions_admin_update
ON public.company_schedule_exceptions FOR UPDATE TO authenticated
USING (public.portal_is_admin()) WITH CHECK (public.portal_is_admin());
CREATE POLICY company_schedule_exceptions_admin_delete
ON public.company_schedule_exceptions FOR DELETE TO authenticated
USING (public.portal_is_admin());

CREATE POLICY company_representatives_admin_select
ON public.company_representatives FOR SELECT TO authenticated
USING (public.portal_is_admin());
CREATE POLICY company_representatives_admin_insert
ON public.company_representatives FOR INSERT TO authenticated
WITH CHECK (public.portal_is_admin());
CREATE POLICY company_representatives_admin_update
ON public.company_representatives FOR UPDATE TO authenticated
USING (public.portal_is_admin()) WITH CHECK (public.portal_is_admin());
CREATE POLICY company_representatives_admin_delete
ON public.company_representatives FOR DELETE TO authenticated
USING (public.portal_is_admin());

CREATE POLICY portal_leads_admin_select
ON public.portal_leads FOR SELECT TO authenticated
USING (public.portal_is_admin());
CREATE POLICY portal_leads_admin_insert
ON public.portal_leads FOR INSERT TO authenticated
WITH CHECK (public.portal_is_admin());
CREATE POLICY portal_leads_admin_update
ON public.portal_leads FOR UPDATE TO authenticated
USING (public.portal_is_admin()) WITH CHECK (public.portal_is_admin());
CREATE POLICY portal_leads_admin_delete
ON public.portal_leads FOR DELETE TO authenticated
USING (public.portal_is_admin());

CREATE POLICY appointment_reservations_admin_select
ON public.appointment_reservations FOR SELECT TO authenticated
USING (public.portal_is_admin());
CREATE POLICY appointment_reservations_admin_insert
ON public.appointment_reservations FOR INSERT TO authenticated
WITH CHECK (public.portal_is_admin());
CREATE POLICY appointment_reservations_admin_update
ON public.appointment_reservations FOR UPDATE TO authenticated
USING (public.portal_is_admin()) WITH CHECK (public.portal_is_admin());
CREATE POLICY appointment_reservations_admin_delete
ON public.appointment_reservations FOR DELETE TO authenticated
USING (public.portal_is_admin());

CREATE POLICY portal_appointments_admin_select
ON public.portal_appointments FOR SELECT TO authenticated
USING (public.portal_is_admin());
CREATE POLICY portal_appointments_admin_insert
ON public.portal_appointments FOR INSERT TO authenticated
WITH CHECK (public.portal_is_admin());
CREATE POLICY portal_appointments_admin_update
ON public.portal_appointments FOR UPDATE TO authenticated
USING (public.portal_is_admin()) WITH CHECK (public.portal_is_admin());
CREATE POLICY portal_appointments_admin_delete
ON public.portal_appointments FOR DELETE TO authenticated
USING (public.portal_is_admin());

CREATE POLICY appointment_checkins_admin_select
ON public.appointment_checkins FOR SELECT TO authenticated
USING (public.portal_is_admin());
CREATE POLICY appointment_checkins_admin_insert
ON public.appointment_checkins FOR INSERT TO authenticated
WITH CHECK (public.portal_is_admin());
CREATE POLICY appointment_checkins_admin_update
ON public.appointment_checkins FOR UPDATE TO authenticated
USING (public.portal_is_admin()) WITH CHECK (public.portal_is_admin());
CREATE POLICY appointment_checkins_admin_delete
ON public.appointment_checkins FOR DELETE TO authenticated
USING (public.portal_is_admin());

/* Audit logs and external submission events are append-only evidence. */
CREATE POLICY portal_audit_logs_admin_select
ON public.portal_audit_logs FOR SELECT TO authenticated
USING (public.portal_is_admin());

CREATE POLICY external_form_events_admin_select
ON public.external_form_events FOR SELECT TO authenticated
USING (public.portal_is_admin());
...
/* Direct table access is denied to anonymous users. Authenticated users receive only
   the permissions required for Masters Ready admin screens; RLS still enforces admin role. */
...
/* Rotate the external webhook credential without exposing write access to the table. */
...
/* Security-definer functions must not inherit an attacker-controlled search path. */
